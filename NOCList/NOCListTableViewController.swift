...
class NOCListTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // set title of the view to "Agents" ????????
        loadNOCList()
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "AgentNameCell", for: indexPath)
        
        let aSelectedAgent = agents[indexPath.row]
        
        cell.textLabel?.text = aSelectedAgent.coverName
        cell.detailTextLabel?.text = aSelectedAgent.realName
        
        if aSelectedAgent.compromised == true {
    ...
    private func loadNOCList() {  // is there a way to quickly make all of the lines with parentheses around them?????????
